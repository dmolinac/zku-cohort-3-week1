...
template sudoku() {
    ...
    // RangeProof components initialization
    component puz_rangeProof[9][9];
    component sol_rangeProof[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            puz_rangeProof[i][j] = RangeProof(32);
            puz_rangeProof[i][j].range[0] <== 0;
            puz_rangeProof[i][j].range[1] <== 9;
            puz_rangeProof[i][j].in <== puzzle[i][j];
            assert(puz_rangeProof[i][j].out);
            sol_rangeProof[i][j] = RangeProof(32);
            sol_rangeProof[i][j].range[0] <== 0;
            sol_rangeProof[i][j].range[1] <== 9;
            sol_rangeProof[i][j].in <== solution[i][j];
            assert(sol_rangeProof[i][j].out);
